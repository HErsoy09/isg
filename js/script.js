...
async function searchNACE() {
    const searchInput = document.getElementById("naceInput").value; // ID düzeltildi
    const resultDiv = document.getElementById("naceResult"); // ID düzeltildi

    try {
        const data = await loadDataNACE(); // Doğru fonksiyon çağrısı
...
async function searchFirma() {
    ...
    const resultDiv = document.getElementById("firmaResult"); // ID düzeltildi

    try {
        const data = await loadDataFirma(); // Doğru fonksiyon çağrısı

        let filteredData = data.filter(item => {
            const matchSozlesme = !sozlesme || item.SozlesmeGirisiYapildiMi === sozlesme;
...
function createTableHTML(data) {
    ...
    data.forEach(item => {
        tableHtml += `
            <tr>
                <td>${item.SozlesmeGirisiYapildiMi === 'TRUE' ? 'Var' : 'Yok'}</td>
